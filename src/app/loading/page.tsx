...
export default function LoadingPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-bbs-bg text-bbs-cyan font-[amiga4ever] text-base">
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-bbs-bg text-bbs-fg font-[amiga4ever]">
      ...
      <div className="w-full max-w-2xl px-4 py-6 border border-bbs-magenta bg-bbs-bg/80 rounded-lg shadow-lg">
        <h2 className="text-xl md:text-2xl font-bold text-bbs-yellow mb-4 font-[amiga4ever]">{sections[sectionIdx]?.label}</h2>
        {showTypewriter ? (
          <TypewriterAnimation
            show={showTypewriter}
            text={sections[sectionIdx]?.content?.replace(/<[^>]+>/g, "") || ""}
          ...
          <span className="whitespace-pre-line text-bbs-fg font-[amiga4ever] text-base md:text-lg lg:text-xl">
            {sections[sectionIdx]?.content?.replace(/<[^>]+>/g, "") || ""}
          </span>
        )}
      </div>
    </div>
